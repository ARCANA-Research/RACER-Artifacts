...
IGNORED!" + normal
    ...
    cycles = statsDict["base_machine"]["xeon_plat_0_0"]["cycles"]
    idle = statsDict["base_machine"]["xeon_plat_0_0"]["thread0"]["cycles_in_pause"]
...
duplicates!" + normal
      ...
      oooDict = statsDict["base_machine"]["xeon_plat_%d_%d" % (i,i)]
...
#    self.updateStat(memElem, "memory_writes", write)
    ...
    mc_access = 0
    for i in range(int(numCores)):
        ctrlComponent = statsDict["base_machine"]["core_%d_cont" %i]["cpurequest"]
        mc_read += ctrlComponent["count"]["hit"]["read"]["hit"] + ctrlComponent["count"]["hit"]["read"]["forward"]
        mc_write += ctrlComponent["count"]["hit"]["write"]["hit"] + ctrlComponent["count"]["hit"]["write"]["forward"]
